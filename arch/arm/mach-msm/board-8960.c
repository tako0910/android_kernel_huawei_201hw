...
#define MSM_PMEM_ADSP_SIZE         0x8200000
...
#ifdef CONFIG_HUAWEI_USBSWITCH
static struct platform_device usbswitch_device = {
	.name = "usbswitch",
	.id = 0,
};
#endif
...
static int wr_phy_init_seq[] = {
	...
	0x4D, 0x81, /* update DC voltage level */
...
#ifdef CONFIG_HUAWEI_USBSWITCH
static struct i2c_board_info msm_usbswitch_boardinfo[] __initdata = {
    {
        I2C_BOARD_INFO("usbswitch_main", 0x25),
		.irq = MSM_GPIO_TO_INT(GPIO_USBSWITCH_INT),
    },
};
#endif
...
static  enum CAMERA_POWRER_SEQ_TYPE camera_pwdseq_type;

enum CAMERA_POWRER_SEQ_TYPE get_camera_power_seq_type(void)
{
   return camera_pwdseq_type;
}

#define CAM_1P2_EN_DCM 25
static struct regulator *cam_vio;
static struct regulator *cam_8921_l10;

#define SMPS_2P85_EN 33           
#define CAM_1P2_EN_U9202L 25
static struct regulator *cam_vaf;
static struct regulator *mipi_csi_vdd;
static struct regulator *cam_vana;


static int enable_camera_power(void)
{
    int rc;
    ...
    if (cam_8921_l10 == NULL) {
        cam_8921_l10 = regulator_get(NULL, "8921_l10");
        if (IS_ERR(cam_8921_l10)) {
            printk("%s: 8921_l10 get failed\n", __func__);
            cam_8921_l10 = NULL;
            goto error_out;
        }
        if (regulator_set_voltage(cam_8921_l10, 2700000,
			2700000)) {
			printk("%s: 8921_l10 set voltage failed\n",
				__func__);
            goto docom_8921_l10_put;
        }
        if (regulator_set_optimum_mode(cam_8921_l10,
			85600) < 0) {
			printk("%s: 8921_l10 set optimum mode failed\n",
				__func__);
            goto docom_8921_l10_set_voltage;
        }
        if (regulator_enable(cam_8921_l10)) {
			printk("%s: 8921_l10 enable failed\n", __func__);
            goto docom_8921_l10_set_optimum_mode;
        }  		
    }
	mdelay(2);
    //step 1 : CAM_ISP_1P2_EN
    rc = gpio_request(PM8921_GPIO_PM_TO_SYS(CAM_1P2_EN_DCM), "CAM_ISP_1P2_EN");
    rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(CAM_1P2_EN_DCM), &param);
    rc = gpio_direction_output(PM8921_GPIO_PM_TO_SYS(CAM_1P2_EN_DCM), 1); 
    mdelay(2);
    if (cam_vio == NULL) {
        cam_vio = regulator_get(NULL, "8921_lvs5");
        if (IS_ERR(cam_vio)) {
			printk("%s: VREG VIO get failed\n", __func__);
			cam_vio = NULL;
			goto cam_ISP_1P2_disable;
		}
		if (regulator_enable(cam_vio)) {
			printk("%s: VREG VIO enable failed\n", __func__);
			goto cam_vio_put;
		}  		
	}
    mdelay(2);
...
int power_seq_enable_softbank_power(void)
{
	...
	if (mipi_csi_vdd == NULL) {
		mipi_csi_vdd = regulator_get(NULL, "8921_l2");
		if (IS_ERR(mipi_csi_vdd)) {
			printk("%s: VREG MIPI CSI VDD get failed\n", __func__);
			mipi_csi_vdd = NULL;
			return -ENODEV;
		}
		if (regulator_set_voltage(mipi_csi_vdd, 1200000,
			1200000)) {
			printk("%s: VREG MIPI CSI VDD set voltage failed\n",
				__func__);
			goto mipi_csi_vdd_put;
		}
		if (regulator_set_optimum_mode(mipi_csi_vdd,
			20000) < 0) {
			printk("%s: VREG MIPI CSI set optimum mode failed\n",
				__func__);
			goto mipi_csi_vdd_set_voltage;
		}
		if (regulator_enable(mipi_csi_vdd)) {
			printk("%s: VREG MIPI CSI VDD enable failed\n",
				__func__);
    ...
    //CAM_ISP_1P2_EN
    rc = gpio_request(PM8921_GPIO_PM_TO_SYS(25), "CAM_ISP_1P2_EN");
    rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(25), &param);
    rc = gpio_direction_output(PM8921_GPIO_PM_TO_SYS(25), 1);
    mdelay(1);
	...
	if (cam_vana == NULL) {
		...
		if (IS_ERR(cam_vana)) {
			printk("%s: VREG CAM VANA get failed\n", __func__);
			cam_vana = NULL;
			goto cam_vio_disable;
		}
		if (regulator_set_voltage(cam_vana, 2800000,
			2850000)) {
			printk("%s: VREG CAM VANA set voltage failed\n",
				__func__);
			goto cam_vana_put;
		}
		if (regulator_set_optimum_mode(cam_vana,
			85600) < 0) {
			printk("%s: VREG CAM VANA set optimum mode failed\n",
				__func__);
			goto cam_vana_set_voltage;
		}
		if (regulator_enable(cam_vana)) {
			printk("%s: VREG CAM VANA enable failed\n", __func__);
	...
	if (cam_vaf == NULL) {
		...
		if (IS_ERR(cam_vaf)) {
			printk("%s: VREG CAM VAF get failed\n", __func__);
			cam_vaf = NULL;
			goto cam_vana_disable;
		}
		if (regulator_set_voltage(cam_vaf, 2800000,
			2800000)) {
			printk("%s: VREG CAM VAF set voltage failed\n",
				__func__);
			goto cam_vaf_put;
		}
		if (regulator_set_optimum_mode(cam_vaf,
			300000) < 0) {
			printk("%s: VREG CAM VAF set optimum mode failed\n",
				__func__);
			goto cam_vaf_set_voltage;
		}
		if (regulator_enable(cam_vaf)) {
			printk("%s: VREG CAM VAF enable failed\n", __func__);
...
cam_vana_set_voltage:
    regulator_set_voltage(cam_vana, 0, 2850000);
cam_vana_put:
...
void power_seq_disable_softbank_power(void)
{
    if (cam_vaf) {
        regulator_set_voltage(cam_vaf, 0, 2800000);
        regulator_set_optimum_mode(cam_vaf, 0);
...
static int power_seq_enable_u9202l_power(void)
{
    ...
    rc = gpio_direction_output(PM8921_GPIO_PM_TO_SYS(CAM_1P2_EN_U9202L), 1);
    mdelay(1);
...
static void power_seq_disable_u9202l_power(void)
{
    ...
    mdelay(1); 
    gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(CAM_1P2_EN_U9202L),0);
    usleep_range(1000,2000);
    gpio_free(PM8921_GPIO_PM_TO_SYS(CAM_1P2_EN_U9202L));
    mdelay(1);
    if (cam_vaf) {
...
static int power_seq_enable_c8869l_power(void)
{ 
    int rc;
    //prepare
    struct pm_gpio param = {
	...
	if (cam_vio == NULL) {
		cam_vio = regulator_get(NULL, "8921_lvs5");
		if (IS_ERR(cam_vio)) {
			printk("%s: VREG VIO 8960_lvs5 get failed\n", __func__);
			cam_vio = NULL;
			goto c8869l_mipi_csi_vdd_disable;
	    }

		if (regulator_enable(cam_vio)) {
			printk("%s: VREG VIO  8960_lvs5 enable failed\n", __func__);
    ...
    //S4
	if (cam_vdig_2 == NULL) {
		cam_vdig_2 = regulator_get(NULL, "8921_s4");
		if (IS_ERR(cam_vdig_2)) {
			printk("%s: VREG CAM VDIG  8921_s4 get failed\n", __func__);
			cam_vdig_2 = NULL;
		 	goto c8869l_cam_vio_disable;
		}
		if (regulator_set_voltage(cam_vdig_2, 1800000,
			1800000)) {
			printk("%s: VREG CAM VDIG  8921_s4 regulator_set_voltage failed\n", __func__);
			goto c8869l_cam_vdig_2_put;
		}
		if (regulator_set_optimum_mode(cam_vdig_2,
			100000) < 0) {
			printk("%s: VREG CAM VDIG  8921_s4 regulator_set_optimum_mode failed\n", __func__);
			 goto c8869l_cam_vdig_2_set_voltage;
		}
		if (regulator_enable(cam_vdig_2)) {
			printk("%s: VREG CAM VDIG 8921_s4 enable failed\n", __func__);
			goto c8869l_cam_vdig_2_set_optimum_mode;
		}
	}
    //CAM_1P85_EN_VRZ
    rc = gpio_request(PM8921_GPIO_PM_TO_SYS(25  ), "CAM_1P85_EN_VRZ"); 
    rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(25), &param);
    rc = gpio_direction_output(PM8921_GPIO_PM_TO_SYS(25), 1);   
    ...
    mdelay(1);
       //VDVDD LVS1
    if (cam_vio_2 == NULL) {
        cam_vio_2 = regulator_get(NULL, "8921_lvs1");            
        if (IS_ERR(cam_vio_2)) {
            printk("%s: VREG VIO cam_vio_2 get failed\n", __func__);
            cam_vio_2 = NULL;
            goto c8869l_cam_vana_disable;
        }
        if (regulator_enable(cam_vio_2)) {
            printk("%s: VREG VIO  cam_vio_2 enable failed\n", __func__);            
...
c8869l_cam_vdig_2_set_voltage:
    regulator_set_voltage(cam_vdig_2, 0, 1200000);
...
static void power_seq_disable_c8869l_power(void)
{ 
    int ret;    
...
static void __init register_i2c_devices(void)
{
	...
	int u9201l_front_camera_id_gpio = 0;
	int u9201l_front_camera_id_value = 0;

	int c8869l_camera_id_gpio = 0;
	int c8869l_camera_id_value = 0;
	int c8869l_front_camera_id_gpio = 0;
	int c8869l_front_camera_id_value = 0;

#ifdef CONFIG_I2C
	u8 mach_mask = 0;
	int i;
#ifdef CONFIG_MSM_CAMERA
	struct i2c_registry docomo_camera_12m_liteon_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		docomo_camera_12m_liteon_board_info.board_info,
		docomo_camera_12m_liteon_board_info.num_i2c_board_info,
	};

	struct i2c_registry docomo_camera_12m_liteon_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		docomo_camera_12m_liteon_1p3m_byd_board_info.board_info,
		docomo_camera_12m_liteon_1p3m_byd_board_info.num_i2c_board_info,
	};

	struct i2c_registry docomo_camera_12m_sunny_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		docomo_camera_12m_sunny_board_info.board_info,
		docomo_camera_12m_sunny_board_info.num_i2c_board_info,
	};

	struct i2c_registry docomo_camera_12m_sunny_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		docomo_camera_12m_sunny_1p3m_byd_board_info.board_info,
		docomo_camera_12m_sunny_1p3m_byd_board_info.num_i2c_board_info,
	};

	struct i2c_registry sbm_camera_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		sbm_camera_board_info.board_info,
		sbm_camera_board_info.num_i2c_board_info,
	};

	struct i2c_registry sbm_camera_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		sbm_camera_1p3m_byd_board_info.board_info,
		sbm_camera_1p3m_byd_board_info.num_i2c_board_info,
	};
    
    struct i2c_registry u9202l_camera_8m_liteon_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		u9202l_camera_8m_liteon_board_info.board_info,
		u9202l_camera_8m_liteon_board_info.num_i2c_board_info,
	};

    struct i2c_registry u9202l_camera_8m_liteon_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		u9202l_camera_8m_liteon_1p3m_byd_board_info.board_info,
		u9202l_camera_8m_liteon_1p3m_byd_board_info.num_i2c_board_info,
	};

    struct i2c_registry u9202l_camera_8m_semco_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		u9202l_camera_8m_semco_board_info.board_info,
		u9202l_camera_8m_semco_board_info.num_i2c_board_info,
	};

    struct i2c_registry u9202l_camera_8m_semco_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		u9202l_camera_8m_semco_1p3m_byd_board_info.board_info,
		u9202l_camera_8m_semco_1p3m_byd_board_info.num_i2c_board_info,
	};
    
	struct i2c_registry verzion_camera_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		verzion_camera_board_info.board_info,
		verzion_camera_board_info.num_i2c_board_info,
	};

	struct i2c_registry c8869l_camera_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		c8869l_camera_board_info.board_info,
		c8869l_camera_board_info.num_i2c_board_info,
	};

	struct i2c_registry c8869l_camera_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		c8869l_camera_1p3m_byd_board_info.board_info,
		c8869l_camera_1p3m_byd_board_info.num_i2c_board_info,
	};

	struct i2c_registry c8869l_camera_5m_semco_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
		c8869l_camera_5m_semco_board_info.board_info,
		c8869l_camera_5m_semco_board_info.num_i2c_board_info,
	};
	
	struct i2c_registry c8869l_camera_5m_semco_1p3m_byd_i2c_devices = {
		I2C_SURF | I2C_FFA | I2C_FLUID | I2C_LIQUID | I2C_RUMI,
		MSM_8960_GSBI4_QUP_I2C_BUS_ID,
