...
#endif
/*Add sensor self adapt*/
static void lis3dh_get_layout(struct lis3dh_acc_data *acc)
{
...
#else
        hrtimer_start(&acc->timer, ktime_set(0, NORMAL_TM), HRTIMER_MODE_REL);
#endif
    }
...
#else
        hrtimer_cancel(&acc->timer);
#endif
        cancel_work_sync(&acc->work);
        lis3dh_acc_device_power_off(acc);
    }
                     ...
                     const char *buf, size_t size)
{
    ...
    acc_delay = acc->pdata->poll_interval;
    if (acc_delay < 10)
        acc_delay = 10;
    acc_delay = acc_delay - 1;
    acc->sesc = acc_delay/1000;
    acc->nsesc = (acc_delay%1000)*1000000;
                  ...
                  const char *buf, size_t size)
{
    ...
    if (strict_strtoul(buf, 10, &val))
        return -EINVAL;
    dev_err(&acc->client->dev, "the value of the g range is: %lu\n",
                val);
    switch(val){
        case 2:
        ...
        case 16:
             ...
             break;
        default:
             dev_err(&acc->client->dev, "invalid g range: %lu\n",
                val);
             return -EINVAL;
        }
    ...
    lis3dh_acc_update_g_range(acc, range);
    mutex_unlock(&acc->lock);
    return size;
}
...
static ssize_t attr_get_accl_data(struct device *dev,
        struct device_attribute *attr,    char *buf)
{
...
#else
    acc = container_of(work,
            struct lis3dh_acc_data,    work);
#endif
    mutex_lock(&acc->lock);
...
static enum hrtimer_restart lis3dh_acc_timer_func(struct hrtimer *timer)
{
    struct lis3dh_acc_data *acc = container_of(timer, struct lis3dh_acc_data, timer);
    queue_work(acc_wq, &acc->work);
    hrtimer_start(&acc->timer, ktime_set(acc->sesc, acc->nsesc), HRTIMER_MODE_REL);
...
static int lis3dh_acc_validate_pdata(struct lis3dh_acc_data *acc)
{
    ...
    acc->sesc = acc->pdata->poll_interval/1000;
    acc->nsesc = (acc->pdata->poll_interval%1000)*1000000;
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
...
#else
    INIT_WORK(&acc->work, lis3dh_acc_input_work_func);
#endif
    ...
    hrtimer_init(&acc->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    acc->timer.function = lis3dh_acc_timer_func;
    acc_wq = create_singlethread_workqueue("acc_wq");
    if (!acc_wq)
        return -ENOMEM;
        ...
        const struct i2c_device_id *id)
{
    ...
    struct lis3dh_acc_data *acc;
    struct lis3dh_acc_platform_data *devpd = client->dev.platform_data;
       int err = -1;
    ...
    if(devpd->get_regulator)
    {
        err = devpd->get_regulator(&client->dev);
        if(err < 0){
            printk(KERN_ERR "lis3dh_acc_probe: get power fail!\n");
        goto exit_check_functionality_failed;
        }
    }
