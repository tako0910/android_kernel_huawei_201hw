...
int usbswitch_is_cradle_attached()
{
    ...
    cradle_gpio = get_gpio_num_by_name(CRADLE_DETECT_GPIO);

    INFO_PRINT("usbswitch_get_cradle_state -E; cradle_gpio=%d\n", cradle_gpio);

    rc = gpio_request(cradle_gpio, CRADLE_DETECT_GPIO);
    if(rc)
    {
        gpio_free(cradle_gpio);
        rc = gpio_request(cradle_gpio, CRADLE_DETECT_GPIO);
        if(rc)
        {
    ...
    cradle_gpio_value= gpio_get_value(cradle_gpio);
