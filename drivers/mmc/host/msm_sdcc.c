...
#ifdef CONFIG_BCMDHD
static struct msmsdcc_host  *g_sdcc4 =NULL;
#endif
...
static int msmsdcc_enable(struct mmc_host *mmc)
{
	...
	/* add patch for DEADLOCK from qualcomm */
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		/* because driver of wifi will be uninstalled when mmc suspend wifi.
		 * in that case, wifi will not work normally when resume wifi.
		 * so wifi should not be suspended when msmsdcc suspend
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		if(SDIO_WLAN_ID != host->pdev_id){
			mmc_resume_host(mmc);
		}else if(!host->clks_on && WIFI_IS_ON == g_wifi_on_flag ){
            msmsdcc_setup_clocks(host, true);
            host->clks_on = 1;
		}
#else
